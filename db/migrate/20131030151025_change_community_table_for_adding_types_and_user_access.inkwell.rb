...
class ChangeCommunityTableForAddingTypesAndUserAccess < ActiveRecord::Migration
  def change
    if ::Inkwell::Engine::config.respond_to?('community_table')
      ...
      add_column ::Inkwell::Engine::config.community_table, :writers_ids, :text
      add_column ::Inkwell::Engine::config.community_table, :banned_ids, :text
      add_column ::Inkwell::Engine::config.community_table, :muted_ids, :text
      add_column ::Inkwell::Engine::config.community_table, :invitations_uids, :text
